...
class Event:
    def __init__(self, id, activity_group_name, date, location_id, max_participants, 
                 cost, registration_required, registration_deadline):
        self.id = id
    ...
    @staticmethod
    def create(activity_group_name, date, location_id, max_participants, 
               cost, registration_required, registration_deadline):
        db = get_db()
        cursor = db.cursor()
        cursor.execute(
            '''INSERT INTO events (activity_group_name, date, location_id, 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (activity_group_name, date, location_id, max_participants,
             cost, registration_required, registration_deadline)
        )
        db.commit()
        ...
        db.execute(
            '''UPDATE events
               SET activity_group_name = ?,
                   ...
                   registration_deadline = ?
               WHERE id = ?''',
            (self.activity_group_name, self.date, self.location_id,
             self.max_participants, self.cost, self.registration_required,
             self.registration_deadline, self.id)
        )
        db.commit()
